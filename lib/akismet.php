...
class Akismet {
	...
	/**
	 * @param	string	$siteURL			The URL of your blog.
	 * @param	string	$akismetAPIKey	WordPress API key.
	 */
	public function __construct($siteURL, $akismetAPIKey) {
	...
	public function isKeyValid() {
		...
		$response = $this->sendRequest('key=' . $this->akismetAPIKey . '&blog=' . $this->siteURL, $this->akismetServer, '/' . $this->akismetVersion . '/verify-key');
		writeNote( $response[0] . " || " . $response[1] );
	...
	public function isCommentSpam() {
		...
		if($response[1] == 'invalid' && !$this->isKeyValid()) {
			$errMsg = 'The Wordpress API key passed to the Akismet constructor is invalid.  Please obtain a valid one from http://wordpress.com/api-keys/';
			writeNote( $errMsg, true );
...
class SocketWriteRead implements AkismetRequestSender {
	...
	public function send($host, $port, $request, $responseLength = 1160) {
		...
		if($this->errorNumber != 0) {
			$errMsg = 'Error connecting to host: ' . $host . ' Error number: ' . $this->errorNumber . ' Error message: ' . $this->errorString;
			writeNote($errMsg, true);
			throw new Exception($errMsg);
		}
